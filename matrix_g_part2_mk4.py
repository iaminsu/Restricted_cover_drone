...
#Don't save straigh path: only make indication it is straight. SA part should be coherently modified (Mk6) 
...
class convexMatrix():
        ...
        if indi == "FF":
            for i in self.originPoints:
                ...
                for j in self.originPoints:
                    if not i == j:     
                        ...
                        if result[0] == 1:
                            results_d[FID_i][FID_j] = result[1:]
                            #r_result = tuple([FID_j]) + result[1:]
                            #results_d[FID_i].append(r_result)
                    FID_j += 1
                FID_i += 1
            ...
            for i in self.originPoints:
                ...
                for j in self.destinationPoints:
                    result = self.createConvexPath((i, j), str(FID_i) + "_"+ str(j))
                    if result[0] == 1:
                        r_result = tuple([FID_j]) + result[1:]
                        results_d[FID_i].append(r_result)
                    FID_j += 1
    ...
    def createConvexPath(self, pair, FID_ij):
        ...
        for obs in self.obstaclesPolygons:
            if st_line.crosses(obs):
                st_indi = False
                break
        if st_indi == True:
            #print "self_indi = True"
            if self.indi == "FF":
                return 1, st_line.length, "straight"
            elif self.indi == "FD":
                return 1, st_line.length, None
        else:
